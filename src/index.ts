...
async function main() {
    ...
    // get project index
    const projectIndex: string = await ask.prompt('\nSelect a project (enter number): ');
    ...
    if (selectedIndex >= 0 && selectedIndex < projects.length) {
        ...
        if (selectedProject) {
            const ticketId: string = await ask.prompt('\nJira ticket id (ONE-XXXXX or XXXXX): ');
            ...
            if (auditResult) {
                ...
                if (packagesToUpdate.length > 0) {
                    brancher.create(projectCwd, brancher.name(ticketId));
